...
export default function Messages({ user }) {
    ...
    const [prikazaniPoraki, namestiPrikazaniPoraki] = useState([]);
        ...
        <div className="chat-messages-wrap container-small">
            {
                aktivniPoraki.map((poraka, indexPoraka) => {
                    let najdeno = false;
                    for (let p = 0; p < prikazaniPoraki.length; p++) {
                        if (prikazaniPoraki[p]._id == poraka._id) {
                            najdeno = true;
                        }
                    }
                    if (najdeno == false) {
                    ...
                    namestiPrikazaniPoraki([...prikazaniPoraki, poraka]);
                })
            }
